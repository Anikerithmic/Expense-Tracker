...
const forgotPassword = document.getElementById("forgotpassword");
        forgotPassword.addEventListener('submit',async (e)=>{
            ...
            const response = await axios.post('http://localhost:4000/password/verification',{name:name, email:email});
            if(response.status === 200||response.data.success === true){
                //console.log('has pending finished?');
                const responseOfSMTP = await axios.post('http://localhost:4000/password/resetpassword',{email: email});
